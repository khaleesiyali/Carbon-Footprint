...
var getScrollAncestor = (node3) => {
  while (node3) {
    ...
    if (!node3 || node3 === document.body || !node3.parentNode) return;
    const {
      overflow,
      overflowX,
      overflowY
    } = getComputedStyle(node3);
    if (/auto|scroll|overlay|hidden/.test(overflow + overflowY + overflowX)) return node3;
  }
};
  ...
  const dispatch = (0, import_react5.useCallback)((actionType, item, nextIndex) => {
    ...
    switch (actionType) {
      ...
      case HoverActionTypes.DECREASE:
        ...
        break;
      default:
        if (true) throw new Error(`[React-Menu] Unknown hover action type: ${actionType}`);
    }
    if (!newItem) index = -1;
...
// node_modules/@szhsin/react-menu/dist/es/positionUtils/getNormalizedClientRect.js
var getNativeDimension = (transformed, untransformed) => Math.round(transformed) === untransformed ? transformed : untransformed;
var getNormalizedClientRect = (element2) => {
  const rect = element2.getBoundingClientRect();
  rect.width = getNativeDimension(rect.width, element2.offsetWidth);
  rect.height = getNativeDimension(rect.height, element2.offsetHeight);
  return rect;
};
...
var MenuItem = withHovering("MenuItem", function MenuItem2({
  ...
  const handleClick = (e) => {
    ...
    if (e.key !== void 0) event.key = e.key;
